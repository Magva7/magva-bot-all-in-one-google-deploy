...
import time  # для перезапуска бота в случае ошибки
import os
...
def handle_text(message):
    if message.text == 'ping':  # если юзер написал ping текстом
        ...
        print('Запрошена и отправлена цитатка с баша')
...
def check_kurs_mir():
    ...
    # Ищем все теги <p> с атрибутом style="text-align: center;"
    p_tags  = page2.find_all('p', {'style': 'text-align: center;'})

    # нам нужен 5-ый по списку, записываем его в переменную
    tenge_kurs_string = p_tags[4].text
    tenge_kurs_list_view = re.findall(r'0,\d{4}', tenge_kurs_string) # ищем в строке цифры
    tenge_kurs_str = tenge_kurs_list_view[0]  # берем первый элемент из списка
    tenge_kurs_str = tenge_kurs_string.replace(',', '.')  # меняем запятую на точку
    ...
    tenge_kurs_result = 'За 1 рубль при снятии с банкомата с карты МИР дают ' + str(tenge_kurs_num) + ' тенге'
...
def check_kurs_mig():
    ...
    tenge_kurs_mig_result = 'За 1 рубль в обменнике МИГ сейчас дают ' + tenge_kurs_mig_str + ' тенге'
